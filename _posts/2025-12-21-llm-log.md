...
## 결론: 에이전트를 신뢰할 수 있게 만드는 것

LLM 에이전트의 진정한 프로덕션 진입 조건은 '성능'이 아니라 **'설명 가능성'**이다.

H.E.A.R. 원칙을 요약하면 이렇다.

| 원칙 | 핵심 질문 | 구현 방법 |
|---|---|---|
| **Human-Readable** | 사람이 빠르게 이해할 수 있는가? | `narrative` 필드: 에이전트가 쓰는 자연어 요약 |
| **Event-Sourced** | 과거 상태를 재구성할 수 있는가? | Append-only 로그 스트림 (Kafka/Redpanda) |
| **Attribution** | 이 결과의 책임 소재는 어디인가? | 모델 버전, 프롬프트 버전, 데이터 출처 명시 |
| **Reproducibility** | 동일한 상황을 재현할 수 있는가? | 입력/출력 스냅샷 + 환경 변수 기록 |

감사 시스템을 구축하는 것이 추가 비용처럼 느껴질 수 있다. 하지만 규제 환경이 강화되고, AI의 의사결정 영역이 금융·의료·법률로 확장될수록 이 로그가 제품의 생존을 결정하는 핵심 자산이 된다. 에이전트를 만드는 것만큼, **에이전트를 감시하는 시스템**을 함께 설계하는 것이 진정한 엔지니어링이다.