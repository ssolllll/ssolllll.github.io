...
LLM Model 서비스를 개발하다 보면 모델 자체의 추론 속도뿐만 아니라, 전후처리 파이프라인의 효율성이 전체 시스템 성능을 좌우하는 경우를 자주 마주합니다.
...
## 5. Counter: 토큰 빈도 분석과 어휘 통계

LLM 서비스에서 어떤 단어나 토큰이 얼마나 자주 등장하는지 집계하는 일은 생각보다 자주 필요하다. 프롬프트 품질 분석, 유저 인텐트 파악, 또는 Fine-tuning 데이터셋의 어휘 분포 확인이 대표적인 사례다.

### 🔴 Bad Practice: `dict`로 직접 카운팅
```python
token_freq = {}
for token in token_list:
    if token in token_freq:
        ...
        token_freq[token] = 0
```
매 반복마다 Key 존재 여부를 확인하는 로직이 들어가 코드가 장황해진다.

### 🟢 Best Practice: `Counter`
`collections.Counter`는 내부적으로 `defaultdict(int)`에 C 레벨 최적화를 추가한 구조다. 생성과 동시에 카운팅이 완료되며, 상위 N개 요소를 뽑는 `most_common()` 메서드가 내장되어 있다.

```python
from collections import Counter

# 토크나이저 출력 예시
token_list = ["the", "model", "the", "token", "model", "the"]

# 단 한 줄로 빈도 집계 완료
token_freq = Counter(token_list)

# 가장 빈번한 상위 5개 토큰 추출 (어휘 분포 분석)
print(token_freq.most_common(5))
# [('the', 3), ('model', 2), ('token', 1)]

# Counter끼리 합산도 지원 (여러 배치 결과 병합 시 유용)
batch_1 = Counter(["input", "token", "input"])
batch_2 = Counter(["output", "token", "input"])
total = batch_1 + batch_2
print(total)
# Counter({'input': 3, 'token': 2, 'output': 1})
```

**적용 시나리오:**
* Fine-tuning 데이터셋에서 희귀 어휘(Long-tail Token)를 파악하여 특수 토큰 처리 전략 수립.
* 사용자 질의 로그를 분석하여 가장 자주 요청되는 의도(Intent) 파악.
* 프롬프트 내 반복 표현을 탐지하여 `repetition_penalty` 설정의 기준값 도출.
